...
import { downloadMiniAppFile, miniAppProvider } from '@/lib/miniAppHost'
...
export function usePaymentsExport(date?: string) {
  ...
  const handleExport = () => {
    ...
    if (fresh && exportRef.current) {
      ...
      try {
        const ret = downloadMiniAppFile(url, filename)
        const handleOutcome = (r: Awaited<typeof ret>, synchronous = false) => {
          if (r.status === 'completed') {
            showToast({ kind: 'success', text: miniAppProvider() === 'max' ? 'Файл сохранён в папку Max' : 'Файл скачивается' })
          }
          if (r.status === 'failed') {
            console.error('[payments] downloadFile rejected', r.error)
            showToast({ kind: 'error', text: synchronous ? 'Не удалось скачать файл.' : 'Не удалось скачать файл. Попробуйте ещё раз.' })
          }
        }
