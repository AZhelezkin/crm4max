...
type BookingPlace = 'master' | 'client' | 'online'

const BOOKING_PLACE_LABELS: Record<BookingPlace, string> = {
  master: 'Принимаю у себя',
  client: 'Выезд',
  online: 'Онлайн',
}
...
export default function CreateBookingPage() {
  ...
  const [outsideScheduleSlot, setOutsideScheduleSlot] = useState<string | null>(null)
  ...
  const isOutsideSchedule = async (slotDate: string, slotTime: string, durationMinutes: number) => {
    const windows = await getEffectiveWindows(slotDate)
    return !fitsWorkWindow(slotTime, durationMinutes, windows)
  }
  ...
  // Применить слот: абонемент → слот приёма; перенос → reschedule; иначе → подтверждение.
  const applySlot = (s: string) => {
    setTime(s)
  ...
  const onSlotTap = async (s: string) => {
    const slotDuration = packageSessionIndex !== null ? selectedService?.duration ?? TIME_STEP_MIN : durationMin
    if (await isOutsideSchedule(date, s, slotDuration)) {
      setOutsideScheduleSlot(s)
      return
    }
    applySlot(s)
  }
  ...
  const handleAddToCalendar = () => {
    ...
    openAddToCalendar({
      ...
      title: bookingServiceNames(createdBooking),
      date,
      time,
      durationMin: bookingDuration(createdBooking),
      location: createdBooking.onlineMeetingLink
        || createdBooking.clientAddress
        || master?.location
        || '',
    })
  }
  ...
  if (step === 'time') {
    ...
    const visibleDayTimes = date === masterNow.format('YYYY-MM-DD')
      ? dayTimes.filter((t) => hhmmToMin(t) >= currentMinute)
      : dayTimes
      ...
      <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
        ...
        {outsideScheduleSlot && (
          <ConfirmDialog
            ...
            message={`Записать на ${selectedDayjs.format('D MMMM')}, ${outsideScheduleSlot}?`}
    ...
    return (
      <div style={{ minHeight: '100dvh' }}>
        ...
        <div style={{ flex: 1, padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {successOnlineMeetingLink && (
            <button
              type="button"
              onClick={() => openExternalLink(successOnlineMeetingLink)}
              aria-label="Открыть ссылку на онлайн-встречу"
              style={{ ...listItemStyle, width: '100%' }}
            >
              <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ flex: 1, padding: '8px 16px calc(16px + env(safe-area-inset-bottom))', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <FormCard title="Дата и время">
          ...
          {online && (
            <FormRow
              label="Ссылка"
              prompt={!normalizedOnlineMeetingLink}
              stacked={!!trimmedOnlineMeetingLink}
              right={trimmedOnlineMeetingLink ? (
                <div style={{ ...text.body2, color: 'var(--color-on-surface)', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {trimmedOnlineMeetingLink}
                </div>
              ) : undefined}
              value="Добавить"
...
function BookingPlaceMenu({ pos, value, onClose, onSelect }: {
  ...
  value: BookingPlace
  onClose: () => void
  onSelect: (value: BookingPlace) => void
}) {
  ...
  const items = Object.entries(BOOKING_PLACE_LABELS) as [BookingPlace, string][]
  return createPortal(
          ...
          <div key={itemValue}>
            ...
            <button
              ...
              style={{
                ...
                textAlign: 'left', ...text.body2,
                color: itemValue === value ? 'var(--color-active-element)' : 'var(--color-on-surface)',
              }}
            >
              {label}
            </button>
