    ...
    <div style={{ minHeight: '100dvh', background: 'var(--color-background)', paddingBottom: 80 }}>
      ...
      {!master ? (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60dvh', color: 'var(--color-on-surface-secondary)' }}>
          {masterId ? 'Загрузка...' : 'Откройте карточку мастера'}
        </div>
      ) : (
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
          {master.phone && (
            <button
              ...
              style={{
                ...
                background: 'var(--color-surface)', borderRadius: 20, padding: '16px 16px',
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--color-on-surface)' }}>Телефон</div>
                <div style={{ fontSize: 13, color: 'var(--color-on-surface-secondary)', marginTop: 2 }}>{master.phone}</div>
              </div>
            </button>
          )}
          ...
          {master.lat && master.lng && (
            ...
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7Z" stroke="var(--color-primary-surface)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <circle cx="12" cy="9" r="2.5" stroke="var(--color-primary-surface)" strokeWidth="2"/>
              </svg>
              <div>
                <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--color-on-surface)' }}>Адрес</div>
                {master.location && (
                  <div style={{ fontSize: 13, color: 'var(--color-on-surface-secondary)', marginTop: 2 }}>{master.location}</div>
                )}
              </div>
            </button>
          )}

          {!master.phone && !master.lat && (
            <div style={{ textAlign: 'center', color: 'var(--color-on-surface-secondary)', marginTop: 40 }}>
              Мастер не указал контакты
            </div>
          )}
        </div>
      )}
