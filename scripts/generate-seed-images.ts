...
function createColoredPNG(width: number, height: number, hexColor: string): Buffer {
  ...
  // Создаём пиксель данные (RGBA для каждого пикселя)
  ...
  const png = Buffer.concat([
    ...
    // IDAT chunk (simplified - not actually compressed)
...
async function main() {
  try {
    ...
    console.log(`\n📍 Папка с картинками: ${OUTPUT_DIR}\n`)
    console.log('🚀 Дальше можно запустить загрузку командой:')
    console.log(`   cd scripts && tsx seed-service-photos.ts\n`)
  } catch (error) {
