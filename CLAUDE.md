...
Точка входа для клиента — ссылка на бота с параметром `startapp={masterId}`:
```
https://max.ru/id9706002253_bot?startapp={masterId}
```

Тестовые ссылки (seed-данные):
- **Анна Смирнова:** https://max.ru/id9706002253_bot?startapp=ee5e98b7-0a08-4a01-bc38-4e3efaf165d7
- **Дмитрий Козлов:** https://max.ru/id9706002253_bot?startapp=f185d8cb-64d1-4dd1-be90-8e056c220889
...
Модели: `Master`, `MasterPhoto`, `Schedule`, `Category`, `Service`, `ServicePhoto`, `Client`, `Booking`, `Payment`, `Review`
...
`master-app/src/App.tsx` работает в двух режимах:
- Если URL содержит `?startapp=<UUID>` или `#/?masterId=<UUID>` — рендерится **ClientApp** (встроен в `src/client/`)
- Иначе — рендерится **MasterApp**
...
Использует `HashRouter`. Точка входа: `/?masterId=xxx`.
...
- Master использует **`maxUserId`** (поле Max мессенджера, не VK)
- Max Bridge поля у Master: `firstName`, `lastName`, `username`, `languageCode`
...
- Ссылка на бота в Max: `https://max.ru/id9706002253_bot?startapp={masterId}` — параметр `startapp` содержит UUID мастера и пробрасывается в client-app как `masterId`
