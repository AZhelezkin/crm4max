...
- [x] Принято продуктовое решение вызывать Метрику и `setUserID` сразу после silent auth, без отдельного analytics consent gate.
...
- [x] Для backend outcomes создан типизированный адаптер с outbox/worker. HTTP API Метрики не вызывается непосредственно из webhook или CRM write boundary; отдельный bot-адаптер остаётся в этапе 5.
...
- [x] Server-side доставка событий backend реализована через Offline Conversions API с upload/poll lifecycle, retry и идемпотентным outbox; отдельный bot-адаптер остаётся в этапе 5.
...
- [x] Сквозная воронка использует случайный server analytics `UserID`: при non-null ID backend возвращает его после auth, frontend вызывает `setUserID`; исходные provider/CRM ID в Метрику не передаются.
...
При non-null `analyticsUserId` Browser ClientID и server-side visitor ID связываются после silent auth через случайный opaque `UserID`. Связь не содержит Max ID, CRM UUID или PII.
...
- [x] Добавить таблицу/outbox аналитических событий либо использовать существующую надёжную очередь.
- [ ] Создавать событие в той же транзакции, что и изменение бизнес-сущности.
- [x] Добавить идемпотентный ключ, но не передавать его в параметры отчётов Метрики.
- [x] Реализовать повторные попытки и dead-letter обработку.
...
- [x] Инициализация счётчика и выбранная политика немедленной отправки после auth.
...
- [x] Outbox/очередь и дедупликация.
- [ ] Оплаты, подписки и жизненный цикл записей.
- [x] При non-null ID связь клиентских и серверных событий через отдельный случайный analytics `UserID` без provider/CRM ID.
...
- [x] Non-null analytics `UserID` устанавливается после успешного auth; frontend-события, включая `auth_failed`, отправляются согласно выбранной политике без отдельного analytics consent gate.
...
- [x] Browser linkage и Offline Conversions проверены на live-счётчике: два upload получили `PROCESSED`, отчёт цели показал два достижения без sampling.
