...
- [ ] Добавить `VITE_YANDEX_METRICA_ID` в конфигурацию окружения. При отсутствии ID аналитика должна работать как `no-op`.
- [ ] Добавить загрузку счётчика в `master-app/src/main.tsx` либо отдельный компонент `MetricsProvider`.
- [ ] Создать `master-app/src/lib/metrics.ts` с типизированными функциями `trackEvent(name, params)` и `trackPageView(path, params)`.
- [ ] Не обращаться к `window.ym` непосредственно из страниц и компонентов.
- [ ] Очередь событий до готовности счётчика ограничить по размеру; ошибки Метрики не должны влиять на пользовательские сценарии.
- [ ] Добавить dev-режим, который пишет события в консоль без отправки.
- [ ] Перед включением счётчика проверить требования к согласию и обновить тексты в `ConsentsPage` и юридических документах.
- [ ] Выбрать один счётчик для обоих режимов и передавать параметр `app_mode: master | client`. Разные счётчики использовать только если отчёты должны быть полностью изолированы.
...
- [ ] `metrics.ts`, типы событий, `no-op`, dev logger.
- [ ] Инициализация счётчика и политика согласия.
- [ ] SPA page views для BrowserRouter и HashRouter.
- [ ] Unit-тесты: нет ID, счётчик не загружен, параметры фильтруются, отправка не бросает исключения.
...
- [ ] События не отправляются до принятия требуемого согласия.
- [ ] В development/test нет реальных запросов к Яндекс Метрике.
- [ ] Одно действие создаёт одно событие, включая React StrictMode.
- [ ] Успешное событие не отправляется при ошибке API.
- [ ] Названия событий и ключей стабильны и типизированы.
- [ ] Параметры имеют ограниченную кардинальность.
- [ ] В payload нет PII, URL с ID, координат и свободного текста.
- [ ] AdBlock или недоступность Метрики не ломают приложение.
- [ ] Page views корректно работают и в `BrowserRouter`, и в `HashRouter`.
