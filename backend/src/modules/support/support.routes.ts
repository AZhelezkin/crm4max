...
export interface MaxUserInfo {
  chatId: number | null
  displayName: string
  /** Через какого бота с пользователем общаться. null, если пользователь не найден. */
  role: BotRole | null
}

/** Находит chatId, имя и роль пользователя Max по maxUserId.
...
export async function resolveMaxUser(maxUserId: string): Promise<MaxUserInfo> {
  ...
  if (master) {
    const name = [master.firstName, master.lastName].filter(Boolean).join(' ').trim()
      ...
      || 'Пользователь'
    return {
      chatId: master.chatId ? Number(master.chatId) : null,
      displayName: name,
      role: 'master',
    }
  }
  ...
  if (client) {
    return {
      chatId: client.chatId ? Number(client.chatId) : null,
      displayName: client.name || 'Клиент',
      role: 'client',
    }
  }

  return { chatId: null, displayName: 'Пользователь', role: null }
}

export const SUPPORT_PROMPT = 'Опишите проблему, мы получим обращение и обязательно свяжемся с вами'

export async function supportRoutes(app: FastifyInstance) {
  // ── POST /api/support/start — пользователь дёргает из миниаппы ──────────────
  app.post('/start', { onRequest: [authenticate] }, async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const { chatId, role } = await resolveMaxUser(user.maxUserId)
    // Если в БД не нашли — берём роль из JWT (на случай, когда пользователь только-только
    // прошёл silent-auth, но ещё не активировал бота).
    const effectiveRole: BotRole = role ?? user.role ?? 'client'
    ...
    await sendMessage(effectiveRole, chatId, SUPPORT_PROMPT)

    return { ok: true, botUrl: `https://max.ru/${nameFor(effectiveRole)}` }
  })
  ...
  app.post('/reply', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const { chatId, role } = await resolveMaxUser(String(userIdNum))
    if (!chatId || !role) {
    ...
    await sendMessage(role, chatId, text)
    req.log.info({ event: 'support.reply.delivered', role, chatId, userId: userIdNum, len: text.length }, 'support reply forwarded')
